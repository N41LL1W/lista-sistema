document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        const mostrarManager = () => {
            console.log("Função mostrarManager chamada.");
        ...
        const mostrarEditor = (listaId, nomeLista) => {
            console.log(`Função mostrarEditor chamada. Lista ID: ${listaId}`);
        ...
        const mostrarCompra = (nomeLista) => {
            console.log(`Função mostrarCompra chamada. Nome da Lista: ${nomeLista}`);
        ...
        const carregarItensDaLista = async (listaId) => {
            console.log(`Carregando itens da lista ID: ${listaId}`);
        ...
        const adicionarItem = async (listaId, nomeItem) => {
            ...
            console.log(`Adicionando item "${nomeItem}" na lista ID: ${listaId}`);
            const response = await fetch(`/api/listas/${listaId}/itens`, {
                method: 'POST',
        ...
        const adicionarItemEmCompra = async (listaId, nomeItem) => {
            if (!nomeItem.trim()) return;
            console.log(`Adicionando item em compra: "${nomeItem}" na lista ID: ${listaId}`);
            const response = await fetch(`/api/listas/${listaId}/itens`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ nome_item: nomeItem }),
        ...
        const atualizarItem = async (id, valor, quantidade) => {
            console.log(`Atualizando item ID: ${id} com valor: ${valor} e quantidade: ${quantidade}`);
        ...
        console.error("Um erro fatal ocorreu no script:", error);
    }
});